...
import com.tco.requests.UnregisterUserRequest;
...
public final class MicroServer {
    ...
    private void processRestfulAPIrequests() {
        path("/api", () -> {
            ...
            post("/login", (req, res) -> processHttpAuthenticationRequest(req, res, AuthenticationRequest.class));
            post("/create", (req, res) -> processHttpRequest(req, res, CreateUserRequest.class));
            post("/unregister", (req,res)-> processHttpRequest(req, res, UnregisterUserRequest.class));
        });
    }
    ...
    private String processHttpAuthenticationRequest(spark.Request httpRequest, spark.Response httpResponse, Type requestType) {
        setupResponse(httpResponse);
        String jsonString = httpRequest.body();
        Session associatedSession = httpRequest.session();
        try {
            JSONValidator.validate(jsonString, requestType);
            AuthenticationRequest auth = new Gson().fromJson(jsonString, requestType);
            auth.setSessionManager(sessions);
            auth.authenticatedSession = associatedSession;
            return buildJSONResponse(auth);
        } catch (IOException | BadRequestException e) {
            log.info("Bad Request - {}", e.getMessage());
            httpResponse.status(HTTP_BAD_REQUEST);
        } catch (Exception e) {
